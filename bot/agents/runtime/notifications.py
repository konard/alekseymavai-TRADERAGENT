...
class TelegramDashboard:
    ...
    def get_stats(self) -> dict:
        ...
        if self._stats_command_usage:
            most_used = max(self._stats_command_usage, key=self._stats_command_usage.get)  # type: ignore[arg-type]
        return {
    ...
    async def _cmd_committee(self, args: list[str]) -> str:
        ...
        if data is None:
            return "Data not available"
        return data.get("summary", "No committee decision available")  # type: ignore[no-any-return]
    ...
    def add_alert(
        ...
        if priority == AlertPriority.CRITICAL and self._notification_callback is not None:
            ...
            try:
                ...
                loop.create_task(self._notification_callback(alert))  # type: ignore[arg-type]
            except RuntimeError:
                pass
