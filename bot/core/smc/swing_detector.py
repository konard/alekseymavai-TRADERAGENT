...
def find_swing_points(
    ...
    for i_win in range(len(window_max)):
        ...
        if centre_high == window_max[i_win]:
            results.append(
                SwingPoint(index=i, price=centre_high, swing_type=SwingType.HIGH, strength=strength)
            )

        if centre_low == window_min[i_win]:
            results.append(
                SwingPoint(index=i, price=centre_low, swing_type=SwingType.LOW, strength=strength)
            )
...
def find_swing_highs(high: np.ndarray, low: np.ndarray, strength: int = 5) -> list[SwingPoint]:
    """Convenience wrapper — returns only swing highs."""
    return [s for s in find_swing_points(high, low, strength) if s.is_high]


def find_swing_lows(high: np.ndarray, low: np.ndarray, strength: int = 5) -> list[SwingPoint]:
