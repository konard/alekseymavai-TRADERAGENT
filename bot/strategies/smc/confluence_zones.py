...
class ConfluenceZoneAnalyzer:
    ...
    def _update_zone_status(self, df: pd.DataFrame) -> None:
        """Update zone status based on latest price action.

        Checks each active OB and FVG against the most recent candle to
        determine if zones have been invalidated or filled.
        """
        if len(df) == 0:
            return
        last = df.iloc[-1]
        ...
        for fvg in self.fair_value_gaps:
            ...
            if gap_size <= Decimal("0"):
                continue
            if fvg.is_bullish:
                filled_high = min(last_high, fvg.gap_high)
                filled_low = max(last_low, fvg.gap_low)
            else:
                filled_high = min(last_high, fvg.gap_high)
                filled_low = max(last_low, fvg.gap_low)
            if filled_high > filled_low:
                fill_pct = float((filled_high - filled_low) / gap_size * Decimal("100"))
