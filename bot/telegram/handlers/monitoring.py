...
async def cmd_pnl(message: Message) -> None:
    ...
    if orch.smc_strategy:
        active = orch.smc_strategy.get_active_positions()
        perf = orch.smc_strategy.get_performance()
        closed_count = perf.total_trades
...
async def cmd_positions(message: Message) -> None:
    ...
    if orch.dca_engine and orch.dca_engine.position:
        ...
        response += (
            ...
            f"Amount: {pos.amount}\n"
            f"Steps: {orch.dca_engine.position.step_number}/{orch.dca_engine.max_steps}\n"
        )
    ...
    if orch.smc_strategy:
        ...
        if smc_active:
            ...
            for smc_pos in smc_active[:5]:
                response += (
                    f"ID: `{str(smc_pos.position_id)[:8]}`\n"
